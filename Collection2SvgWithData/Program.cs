...
namespace Collection2SvgWithData
{
    class MainClass
    {
        public static void Main(string[] args)
        {
            ...
            string destFile = basePath + "_Standalone.svg";
            ...
            foreach (KeyValuePair<string, SpriteInfo> item in spriteInfos)
            {
                ...
                string path = Path.Combine(basePath, string.Concat(item.Value.Name, ".png"));

                if (File.Exists(path))
                {
                    svgDataDefs.Add(string.Format(
                        "    <g id=\"{0}\"><image width=\"{1}px\" height=\"{2}px\" xlink:href=\"{3}\"/></g>",
                        item.Value.Name, item.Value.Rect.Width, item.Value.Rect.Height, Base64Encode(path))
                    );
                }
            }
        ...
        private static string Base64Encode(string path)
        {
            ...
            using (MemoryStream ms = new MemoryStream())
            {
                ...
                return string.Concat("data:image/png;base64,", Convert.ToBase64String(imageBytes));
            }
        }
    }
}
